/** biome-ignore-all lint/suspicious/noExplicitAny: zod handles the types */
/** biome-ignore-all lint/correctness/useExhaustiveDependencies: ok here */
...
export const useRealtime = <T extends Record<string, Record<string, unknown>>>({
  ...
  const connect = ({
    reconnect = !isInitialConnectionRef.current,
  }: {
    reconnect?: boolean;
  } = {}) => {
