...
export function PrecisionFlowSection({
	...
	const {
		...
		jumpToStep,
		sectionRef,
	} = usePrecisionFlowPlayback({
		initialPhase,
		autoplay,
	});
	const [composerVisibility, setComposerVisibility] =
		useState<MessageVisibility>("public");
	const scene = useMemo(() => buildPrecisionFlowScene(phase), [phase]);
	...
	useEffect(() => {
		if (phase === PRECISION_FLOW_INITIAL_PHASE) {
	...
	return (
		...
		>
			...
			<div className="relative flex h-full flex-1 flex-col lg:min-h-screen lg:flex-row">
					...
					<div className="flex items-center gap-3">
						<Button
							data-precision-primary-action-icon={
								primaryActionPresentation.icon === PlayIcon ? "play" : "replay"
							}
